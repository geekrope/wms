...
function get_trend(p_growth, p_decline, strong = SIGNIFICANCE_STRONG, weak = SIGNIFICANCE_WEAK) {
    if (!Number.isFinite(p_growth) || !Number.isFinite(p_decline))
        return "❔"; // degenerate fit: no evidence either way
    if (p_growth < strong)
        return "⬆️";
    if (p_decline < strong)
        return "⬇️";
...
function compute_current_regression(x, running_totals, cutoff) {
    const window_x = x.slice(cutoff);
    const window_y = running_totals.slice(cutoff);
    if (window_x.length < MIN_PVALUE_POINTS)
        return undefined; // not enough points for a stable fit
    return compute_regression_line(window_x, window_y, x[0]);
}
function compute_bar_geometry(events) {
...
function compute_shelf_line(x, running_totals) {
    const shelf_x = [];
    const shelf_y = [];
...
function compute_regression_line(x, y, x0) {
...
function lambda_to_cutoff_date(lambda, date_min, date_max) {
    return lambda * date_max + (1 - lambda) * date_min;
}
function build_cutoff_shape(x0) {
    return {
        type: "line",
        name: "Cutoff",
        x0, x1: x0,
...
function build_regression_trace(regression) {
    return [{
            type: "scatter",
            mode: "lines",
            name: "OLS trend",
...
function normal_pdf(x, sigma) {
    return Math.exp(-0.5 * (x / sigma) ** 2) / (sigma * Math.sqrt(2 * Math.PI));
}
function build_normal_pdf_trace(sigma) {
...
function build_residuals_trace(residuals) {
    return {
        type: "histogram",
...
function build_qq_trace(residuals, sigma) {
...
function render_timeline_chart(container, events, geometry, cutoff, regression) {
...
function render_residuals_chart(container, data) {
    if (typeof Plotly === "undefined")
        throw new Error("Plotly library is not loaded.");
    container.innerHTML = "";
    if (!data || !(data.sigma > 0))
        return; // all deltas equal => sigma 0 => nothing to compare against
...
function render_qq_chart(container, data) {
    container.innerHTML = "";
    if (typeof Plotly === "undefined")
        throw new Error("Plotly library is not loaded.");
    if (!data || !(data.sigma > 0))
        return; // qq() divides by sigma
...
async function refresh_current_category(cutoff_slider) {
...
export async function refresh_regression_plot() {
...
//# sourceMappingURL=analytics_linreg.js.map