...
export function assertFactory() {
    ...
    const assert: AssertFunc = (condition, name) => {
        total++;
        if (condition) {
            passed++;
...
async function create_test_database(): Promise<{ manager: DatabaseManager, driver: SqlJsDriver }> {
    const SQL = await window.initSqlJs({
        locateFile: (file: string) => `./src/modules/${file}`
    });
...
async function get_category_id(manager: DatabaseManager, title: string): Promise<number> {
    const categories = await manager.get_categories();
    const found = categories.find(c => c.title === title);
    if (found?.id === undefined) throw new Error(`Category "${title}" not found in test fixture`);
    return found.id;
}

async function get_box_id(manager: DatabaseManager, title: string): Promise<number> {
    const boxes = await manager.get_boxes();
    const found = boxes.find(b => b.title === title);
    if (found?.id === undefined) throw new Error(`Box "${title}" not found in test fixture`);
    return found.id;
}
...
function init_labels<T>(nodes: Node<T>[]): Map<Node<T>, number> {
...
export const GraphTests = {
    ...
    // --- 2. CYCLE DETECTION ---
    testDetectCycleFindsCycle(assert: AssertFunc) {
    ...
    testDetectCycleAcceptsDAG(assert: AssertFunc) {
        const a = new ValueNode("A");
        const b = new ValueNode("B");
        const c = new ValueNode("C");
    ...
    testDetectCycleThrowsOnUnlabeledNeighbor(assert: AssertFunc) {
        const a = new ValueNode("A");
        const b = new ValueNode("B");
...
export const DatabaseTests = {
    ...
    async testInitTables(assert: AssertFunc) {
    ...
    // --- 2. CATEGORIES ---
    async testAddAndGetCategories(assert: AssertFunc) {
        const { manager } = await create_test_database();
        await manager.init_tables();

        await manager.add_categories(
            { id: undefined, title: "Tuna", weight: 200 },
            { id: undefined, title: "Tushonka", weight: 150 }
        );
    ...
    async testRemoveCategorySucceedsAfterItemRemoved(assert: AssertFunc) {
        const { manager } = await create_test_database();
        await manager.init_tables();
        await manager.add_categories({ id: undefined, title: "Fish", weight: 200 });
        await manager.add_boxes({ id: undefined, title: "Shelf", max_load: null });
    ...
    // --- 3. ITEMS ---
    async testAddItemsRejectsUnknownCategory(assert: AssertFunc) {
        const { manager } = await create_test_database();
        await manager.init_tables();
        await manager.add_boxes({ id: undefined, title: "Shelf", max_load: null });
    ...
    async testAddItemsRejectsUnknownBox(assert: AssertFunc) {
        const { manager } = await create_test_database();
        await manager.init_tables();
    ...
    async testGetItemsFiltersRemoved(assert: AssertFunc) {
        const { manager } = await create_test_database();
        await manager.init_tables();
        await manager.add_categories({ id: undefined, title: "Food", weight: 200 });
        await manager.add_boxes({ id: undefined, title: "Shelf", max_load: null });
        await manager.add_items(
            new Item("Food", Date.now() + 1000000, "Shelf", 0),
            new Item("Food", Date.now() + 2000000, "Shelf", 0)
        );
    ...
    async testUpdateItemMovesBoxAndCategory(assert: AssertFunc) {
        const { manager } = await create_test_database();
        await manager.init_tables();
        await manager.add_categories(
            { id: undefined, title: "Canned", weight: 200 },
            { id: undefined, title: "Fresh", weight: 100 }
        );
        await manager.add_boxes(
            { id: undefined, title: "BoxA", max_load: null },
            { id: undefined, title: "BoxB", max_load: null }
        );
    ...
    async testGetBoxWeightsSumsActiveItems(assert: AssertFunc) {
        const { manager } = await create_test_database();
        await manager.init_tables();
        await manager.add_categories(
            { id: undefined, title: "Heavy", weight: 10 },
            { id: undefined, title: "NoWeight", weight: null }
        );
        await manager.add_boxes({ id: undefined, title: "Crate", max_load: null });
        await manager.add_items(
            new Item("Heavy", Date.now() + 1000000, "Crate", 0),
            new Item("Heavy", Date.now() + 1000000, "Crate", 0),
            new Item("NoWeight", Date.now() + 1000000, "Crate", 0)
        );
    ...
    // --- 4. BOX ADJACENCY ---
    async testBoxAdjacencyLifecycle(assert: AssertFunc) {
        const { manager } = await create_test_database();
        await manager.init_tables();
        await manager.add_boxes(
            { id: undefined, title: "A", max_load: null },
            { id: undefined, title: "B", max_load: null }
        );
    ...
    async testRemoveBoxCascadesAdjacency(assert: AssertFunc) {
        const { manager } = await create_test_database();
        await manager.init_tables();
        await manager.add_boxes(
            { id: undefined, title: "A", max_load: null },
    ...
    async testRawQueryAndRunDelegateToDriver(assert: AssertFunc) {
        const calls: { method: string, sql: string, params: SqlParams | undefined }[] = [];

        const fake_driver: IDatabaseDriver = {
            async query_raw(sql, params) {
                calls.push({ method: "query_raw", sql, params });
                return [{ ok: 1 }];
            },
            async query<T>(sql: string, _ctor: Constructor<T>, params?: SqlParams): Promise<T[]> {
...
async function seed_removed_item(
    manager: DatabaseManager,
    category_id: number,
    box_id: number,
    add_date: number,
    remove_date: number | null
): Promise<void> {
    await manager.run_raw(
        `INSERT INTO items (category_id, box_id, expiration_date, status, add_date, remove_date)
        VALUES (:category_id, :box_id, 0, 0, :add_date, :remove_date);`,
        { ":category_id": category_id, ":box_id": box_id, ":add_date": add_date, ":remove_date": remove_date }
    );
}

export const AnalyticsTests = {
    // --- 1. CATEGORY EVENTS ---
    async testGetCategoryEventsGroupsDeltaByDay(assert: AssertFunc) {
    ...
    async testGetCategoryEventsThrowsForUnknownCategory(assert: AssertFunc) {
        const { manager, driver } = await create_test_database();
        await manager.init_tables();
        const analytics = new Analytics(driver, manager);
    ...
    async testGetActivityCountsRemovalsPerDay(assert: AssertFunc) {
        const { manager, driver } = await create_test_database();
        await manager.init_tables();
        await manager.add_categories({ id: undefined, title: "Widgets", weight: 5 });
        await manager.add_boxes({ id: undefined, title: "Shelf", max_load: null });
    ...
    async testGetActivityEmptyRangeReturnsEmpty(assert: AssertFunc) {
        const { manager, driver } = await create_test_database();
        await manager.init_tables();
        await manager.add_categories({ id: undefined, title: "Widgets", weight: 5 });
        await manager.add_boxes({ id: undefined, title: "Shelf", max_load: null });
    ...
    async testGetActivityRespectsDateBoundaries(assert: AssertFunc) {
        const { manager, driver } = await create_test_database();
        await manager.init_tables();
        await manager.add_categories({ id: undefined, title: "Widgets", weight: 5 });
        await manager.add_boxes({ id: undefined, title: "Shelf", max_load: null });
