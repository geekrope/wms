...
function make_svg_element(tag: string, attrs: Record<string, string | number>): SVGElement {
    const el = document.createElementNS("http://www.w3.org/2000/svg", tag);
    for (const [key, value] of Object.entries(attrs)) el.setAttribute(key, String(value));
    return el;
}

function get_heatmap_color(count: number): string {
    return HEATMAP_SCALE[Math.min(count, HEATMAP_SCALE.length - 1)];
}

function make_heatmap_cell(x: number, y: number, count: number, date: Date): SVGRectElement {
    const rect = make_svg_element("rect", { x, y, width: HEATMAP_CELL_SIZE, height: HEATMAP_CELL_SIZE, rx: 2, ry: 2, fill: get_heatmap_color(count) }) as SVGRectElement;
...
function build_heatmap_svg(activity: ActivityCount[], year: number): { width: number, height: number, elements: SVGElement[] } {
...
export async function refresh_activity_plot(container: HTMLElement, year: number): Promise<void> {
    const analytics = get_analytics_object();
    const activity: ActivityCount[] = await analytics.get_activity(year);

    const { width, height, elements } = build_heatmap_svg(activity, year);
